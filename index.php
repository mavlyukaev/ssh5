...
if ($_SERVER['REQUEST_METHOD'] == 'GET') {
  ...
  if (count(array_filter($errors)) === 0 && !empty($_COOKIE[session_name()]) && session_start() && !empty($_SESSION['login'])) {
    ...
    try {
      ...
      $stmt = $db->prepare("SELECT name, email, year, gender, hand, biography FROM application WHERE id = ?");
      $stmt->execute([$app_id]);
      $dates = $stmt->fetchAll(PDO::FETCH_ASSOC);

      $stmt = $db->prepare("SELECT KEY_DATA FROM superwo WHERE KEY_DATA = ?");
      $stmt->execute([$app_id]);
  ...
  if (!empty($_COOKIE[session_name()]) && session_start() && !empty($_SESSION['login'])) {
    ...
    try {
      ...
      $stmt = $db->prepare("SELECT KEY_DATA FROM tables_superwo WHERE id_DATA_FORM = ?");
      $stmt->execute([$app_id]);
      $abil = $stmt->fetchAll(PDO::FETCH_COLUMN, 0);

      if (array_diff($abil, $abilities)) {
        $stmt = $db->prepare("DELETE FROM tables_superwo WHERE id_DATA_FORM = ?");
        $stmt->execute([$app_id]);

        $stmt = $db->prepare("INSERT INTO tables_superwo (id_DATA_FORM, id_DATA) VALUES (?, ?)");
